...
门禁模式下的站点可见性：每个站点有「公开站点」属性（添加结果页与编辑弹窗中勾选，默认公开）。**未持 token 的访客可浏览公开站点并点击跳转**（`GET /api/sites`、`GET /api/tags` 只返回公开站点及其标签，私有站点的标签名也不泄露），但没有任何修改能力；**取消勾选的私有站点仅持 token 的浏览器可见**（图标壳右上角有小锁标识）。

门禁模式下的使用流程：未持 token 的访客打开首页直接看到全部公开站点（只读）；您自己在浏览器点右上角「访问 Token」输入一次，保存在该浏览器的 localStorage 并解锁私有站点与全部管理能力；换浏览器或清除站点数据后重新输入即可。
